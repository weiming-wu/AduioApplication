...
int UpnpDevice::handle_subscription_request(struct upnp_device* priv,
    ...
    LOG(INFO, "Initial variable sync: %s\n", xml_value);
    ...
    if (rc == UPNP_E_SUCCESS) {
        ...
        LOG(ERROR, "Accept Subscription Error: %s (%d)\n", UpnpGetErrorMessage(rc), rc);
    }
...
int UpnpDevice::handle_var_request(struct upnp_device* priv,
    ...
    var_event->ErrCode = (result == NULL)
                         ...
                         : UPNP_E_SUCCESS;
    LOG(INFO, "Variable request %s -> %s (%s)",
...
int UpnpDevice::handle_action_request(struct upnp_device* priv,
    ...
    LOG(INFO, "[action req]--> ServiceID:(%s), ActionName:(%s)\n",
                ar_event->ServiceID,
                ar_event->ActionName);
    ...
    if (event_action == NULL) {
        LOG(ERROR, "Unknown action '%s' for service '%s'",
    ...
    if (event_action->callback) {
        ...
        LOG(ERROR, "Got a valid action, but no handler defined (!)\n"
                                          "  ErrCode:    %d\n"
                                          "  Socket:     %d\n"
                                          "  ErrStr:     '%s'\n"
                                          "  ActionName: '%s'\n"
...
int UpnpDevice::event_handler(Upnp_EventType EventType, void* event, void* userdata) {
    ...
    default:
        LOG(ERROR, "Unknown event type: %d", EventType);
        break;
    }
...
bool UpnpDevice::initialize_device(struct upnp_device_descriptor* device_def,
    ...
    */
    while (UPNP_E_SUCCESS != rc) {
        if (ip_address == NULL) {
            LOG(ERROR, "ip_address is NULL, waite a second\n");
            sleep(1);
            rc = UpnpInit(ip_address, port);
        } else {
            LOG(ERROR, "UpnpInit(ip=%s, port=%d) Error: %s (%d)\n",
                                              ip_address, port, UpnpGetErrorMessage(rc), rc);
            return FALSE;
        }
    }

    LOG(INFO, "Registered IP=%s port=%d\n",
                                     UpnpGetServerIpAddress(), UpnpGetServerPort());

    rc = UpnpEnableWebserver(TRUE);

    if (UPNP_E_SUCCESS != rc) {
        LOG(ERROR, "UpnpEnableWebServer() Error: %s (%d)\n",
                                          UpnpGetErrorMessage(rc), rc);
        return FALSE;
    }
    ...
    if (UPNP_E_SUCCESS != rc) {
        LOG(ERROR, "UpnpAddVirtualDir() Error: %s (%d)\n",
                                          UpnpGetErrorMessage(rc), rc);
        return FALSE;
    }
    ...
    if (UPNP_E_SUCCESS != rc) {
        LOG(ERROR, "UpnpRegisterRootDevice2() Error: %s (%d)\n",
                                          UpnpGetErrorMessage(rc), rc);
        return FALSE;
    }
    ...
    if (UPNP_E_SUCCESS != rc) {
        LOG(ERROR, "Error sending advertisements: %s (%d)\n", UpnpGetErrorMessage(rc), rc);
        return FALSE;
    }
